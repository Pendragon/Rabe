 ...
 *
 * @param {string} research string
 */
User.prototype.getByDBID = function() 
{
...
User.prototype.create = function(name, password, callback)
{
	...
	users.insert({email: name, password:hash, salt:salt}, function(err, doc) {
		if (err) 
			callback(err, null); 
		else 
			callback(null, doc);
 ...
 */
 User.prototype.auth = function(name, password, callback)
 {
	...
	this.getbyname(name, function(err, result) {
		...
		if (result && bcrypt.compareSync(password, result.hash))
			callback(err, result);
		else
			callback(err, null);
	});
