...
/*	var data = "[";
	...
	this._history.forEach(function(mesure) {
		...
		data = data + "[Date(" + mesure.read_at + ")," + mesure.value + "]";
	});
...
{
/*
	var crypted = cryptage.encrypt_obj(data);
	var decrypted = cryptage.decrypt_obj(crypted);
  	if(debug) {
  		decrypted = cryptage.decrypt_obj(crypted);
  		util.log("Data:" + JSON.stringify(data));
  		util.log("decrypted:" + decrypted);
  		util.log("Data encrypted:" + crypted);
  		util.log("Data decrypted:" + JSON.stringify(decrypted));
  		util.log("Name: " + decrypted["name"]);
  	}
